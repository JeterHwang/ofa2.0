...
class MinMaxObserver(_ObserverBase):
    ...
    def __init__(self, dtype=torch.quint8, qscheme=torch.per_tensor_affine,
                 bitwidth=8, log_scale=False, squeeze=0.0, track_quant_stat=False):
        ...
        self.track_quant_stat = track_quant_stat
        self.min_val_stat = torch.tensor([])
        self.max_val_stat = torch.tensor([])
...
class MovingAverageMinMaxObserver(MinMaxObserver):
    ...
    def __init__(self, averaging_constant=0.01, dtype=torch.quint8,
                 qscheme=torch.per_tensor_affine, bitwidth=8, log_scale=False, squeeze=0.0, track_quant_stat=False):
    ...
    def update_batch_stat(self, new_min, new_max):
        ...
        self.min_val_stat.resize_(min_val.shape)
        self.max_val_stat.resize_(max_val.shape)
        self.min_val_stat.copy_(min_val)
        self.max_val_stat.copy_(max_val)
...
class PerChannelMinMaxObserver(_ObserverBase):
    ...
    def __init__(
        ...
        self.track_quant_stat = track_quant_stat
        self.min_vals_stat = torch.tensor([])
        self.max_vals_stat = torch.tensor([])
    ...
    def _forward(self, x_orig):
        ...
        assert out_channel <= self.max_channel
        old_size = min_vals.size(0)
        ...
        if min_vals.numel() == 0 or max_vals.numel() == 0:
            ...
            if out_channel > old_size:
                min_vals.resize_(new_min.shape)
                max_vals.resize_(new_max.shape)
                min_vals[:old_size] = torch.min(new_min[:old_size], min_vals[:old_size])
                max_vals[:old_size] = torch.max(new_max[:old_size], max_vals[:old_size])
                min_vals[old_size:] = new_min[old_size:]
                max_vals[old_size:] = new_max[old_size:]
            else:
                min_vals[:out_channel] = torch.min(new_min, min_vals[:out_channel])
                max_vals[:out_channel] = torch.max(new_max, max_vals[:out_channel])
...
class MovingAveragePerChannelMinMaxObserver(PerChannelMinMaxObserver):
    ...
    def __init__(
        ...
        super(MovingAveragePerChannelMinMaxObserver, self).__init__(
            ...
            bitwidth=bitwidth, log_scale=log_scale, squeeze=squeeze, max_channel=max_channel, track_quant_stat=track_quant_stat)
        self.averaging_constant = averaging_constant

    def update_batch_stat(self, new_min, new_max):
        min_vals = self.min_vals_stat.to(new_min.device)
        max_vals = self.max_vals_stat.to(new_max.device)
        out_channel = new_min.size(0)
        assert torch.isnan(min_vals).sum() == 0
        assert torch.isnan(max_vals).sum() == 0
        assert out_channel <= self.max_channel
        if min_vals.numel() == 0 or max_vals.numel() == 0:
            min_vals = torch.zeros(self.max_channel) 
            max_vals = torch.zeros(self.max_channel) 
            min_vals[:out_channel].copy_(new_min)
            max_vals[:out_channel].copy_(new_max)
        else:
            assert min_vals.device == new_min.device
            min_vals[:out_channel] = min_vals[:out_channel] + self.averaging_constant * (new_min - min_vals[:out_channel])
            max_vals[:out_channel] = max_vals[:out_channel] + self.averaging_constant * (new_max - max_vals[:out_channel])
                
        self.min_vals_stat.resize_(min_vals.shape)
        self.max_vals_stat.resize_(max_vals.shape)
        self.min_vals_stat.copy_(min_vals)
        self.max_vals_stat.copy_(max_vals)
    ...
    def forward(self, x_orig):
        ...
        # batch_mean = y.mean(1, keepdim=True)
        # batch_var = (y - batch_mean) * (y - batch_mean)
        # batch_var = batch_var.mean(1, keepdim=True)
        ...
        if min_vals.numel() == 0 or max_vals.numel() == 0:
            ...
            min_vals[:out_channel] = min_vals[:out_channel] + self.averaging_constant * (new_min - min_vals[:out_channel])
            max_vals[:out_channel] = max_vals[:out_channel] + self.averaging_constant * (new_max - max_vals[:out_channel])
        # min_vals = self.min_vals_stat
        # max_vals = self.max_vals_stat
        # min_vals = new_min if self.training else self.min_vals_stat
        # max_vals = new_max if self.training else self.max_vals_stat
        # min_vals = new_min if self.training and not self.track_quant_stat else self.min_vals_stat
        # max_vals = new_max if self.training and not self.track_quant_stat else self.max_vals_stat

        # if self.track_quant_stat and min_vals.numel() != 0 and max_vals.numel() != 0:
        #     # if torch.distributed.get_rank() == 0:
        #     #     print('Update Stat')
        #     self.update_batch_stat(new_min, new_max, out_channel)
        # else:
        #     # if torch.distributed.get_rank() == 0:
        #     #     print('Just Inference')
        #     if min_vals.numel() == 0 or max_vals.numel() == 0:
        #         min_vals = new_min
        #         max_vals = new_max
        #     else:
        ...
        #             min_vals.resize_(new_min.shape)
        #             max_vals.resize_(new_max.shape)
        #             min_vals[:old_size] = min_vals[:old_size] + self.averaging_constant * (new_min[:old_size] - min_vals[:old_size])
        #             max_vals[:old_size] = max_vals[:old_size] + self.averaging_constant * (new_max[:old_size] - max_vals[:old_size])
        #             min_vals[old_size:] = new_min[old_size:]
        #             max_vals[old_size:] = new_max[old_size:]
        #         else:
        #             min_vals[:out_channel] = min_vals[:out_channel] + self.averaging_constant * (new_min - min_vals[:out_channel])
