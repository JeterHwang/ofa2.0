...
class ImagenetDataProvider(DataProvider):
	DEFAULT_PATH = '/home/ntu329/Documents/Datasets/ImageNet'
	...
	def __init__(
		...
		# Set self.active_img_size, valid_transforms, train_loader_class 
		if not isinstance(self.image_size, int):
			from utils.my_dataloader import MyDataLoader
			assert isinstance(self.image_size, list)
			self.image_size.sort()  # e.g., 160 -> 224
			MyRandomResizedCrop.IMAGE_SIZE_LIST = self.image_size.copy()
			MyRandomResizedCrop.ACTIVE_SIZE = max(self.image_size)
			
			for img_size in self.image_size: 				# Validation Transform
		...
		if train_size is not None:
			if 0 < train_size <= 1: # Valid_size in the form of ratio
				train_size = int(len(train_dataset) * train_size)
		if valid_size is not None: # Has Validation Dataset
			if not isinstance(valid_size, int): # Valid_size in the form of ratio
				assert isinstance(valid_size, float) and 0 < valid_size < 1
				valid_size = int(len(train_dataset) * valid_size) if train_size is None else int(train_size * valid_size)
			print(train_size, valid_size)
			# Validation set share the same dataset with Training set but only sample a small proportion
			valid_dataset = self.train_dataset(valid_transforms)
			train_indexes, valid_indexes = self.random_sample_valid_set(len(train_dataset), valid_size, train_size)

			if num_replicas is not None: 	# Distributed Sampler
			...
			train_indexes, _ = self.random_sample_valid_set(len(train_dataset), 0, train_size)
			if num_replicas is not None: # Distributed sampler
				train_sampler = MyDistributedSampler(train_dataset, num_replicas, rank, True, np.array(train_indexes))
				self.train = train_loader_class(
					train_dataset, 
			...
			self.valid = None			# No Validation Dataloader
	...
	def save_path(self):
		if self._save_path is None:
			...
			if not os.path.exists(self._save_path):
				self._save_path = os.path.join(os.path.abspath('..') + '\\dataset\\imagenet')
