...
from utils.layers import MBConvLayerQuant
from utils import MyModule, val2list, get_net_device, build_activation, make_divisible, SEModule, MyNetwork
...
from .dynamic_intrinsic import DynamicConvBn2DNonFused, DynamicSepConvBn2DNonFused, DynamicSeparableConvBn2dQuant, DynamicConvBn2dQuant
...
def adjust_fake_quant_according_to_idx(fq : FakeQuantize, idx):
    fq.activation_post_process.min_vals_stat.data = torch.index_select(fq.activation_post_process.min_vals_stat.data, 0, idx)
    fq.activation_post_process.max_vals_stat.data = torch.index_select(fq.activation_post_process.max_vals_stat.data, 0, idx)

def copy_bn(target_bn, ofa_bn):
    feature_dim = target_bn.num_channels if isinstance(target_bn, nn.GroupNorm) else target_bn.num_features

    target_bn.weight.data.copy_(ofa_bn.weight.data[:feature_dim])
    target_bn.bias.data.copy_(ofa_bn.bias.data[:feature_dim])
    if type(ofa_bn) in [nn.BatchNorm1d, nn.BatchNorm2d]:
...
class DynamicLinearLayer(nn.Module):
    ...
    def get_active_subnet(self, in_features, preserve_weight=True):
        ...
        # return a new LinearLayer
        if not preserve_weight:
            return sub_layer
        
        # copy trained wegith from supernet's subnet 
...
class DynamicMBConvLayerQuant(MyModule):
    def __init__(
        self, 
        in_channel_list, 
        out_channel_list,
        ...
        act_func='relu6', 
        use_se=False,
        inv_weight_quant='SD4_per_channel',
        inv_act_quant='int8',
        sep_weight_quant='SD4_per_channel',
        sep_act_quant='int8',   
    ):
        super(DynamicMBConvLayerQuant, self).__init__()
        ...
        self.kernel_size_list   = val2list(kernel_size_list)
        self.expand_ratio_list  = val2list(expand_ratio_list)
        self.inv_weight_quant   = val2list(inv_weight_quant)
        self.inv_act_quant      = val2list(inv_act_quant)
        self.sep_weight_quant   = val2list(sep_weight_quant)
        self.sep_act_quant      = val2list(sep_act_quant)
        ...
        if max(self.expand_ratio_list) == 1:
            ...
            self.inverted_bottleneck = nn.Sequential(OrderedDict([
                ('conv', DynamicConvBn2dQuant(max(self.in_channel_list), max_middle_channel, weight_quant_list=self.inv_weight_quant, act_quant_list=self.inv_act_quant)),
                ('act', build_activation(self.act_func))
            ]))
        # seperable convolution
        self.depth_conv = nn.Sequential(OrderedDict([
            ('conv', DynamicSeparableConvBn2dQuant(max_middle_channel, self.kernel_size_list, self.sep_weight_quant, self.sep_act_quant, self.stride)),
            ('act', build_activation(self.act_func))
        ]))
        if self.use_se:
            self.depth_conv.add_module("se", DynamicSE(max_middle_channel))
        # bottleneck
        self.point_linear = nn.Sequential(OrderedDict([
            ('conv', DynamicConvBn2dQuant(max_middle_channel, max(self.out_channel_list), self.inv_weight_quant, self.inv_act_quant)),
        ]))

        self.active_kernel_size = max(self.kernel_size_list)
        self.active_expand_ratio = max(self.expand_ratio_list)
        self.active_out_channel = max(self.out_channel_list)
        self.active_inv_wquant  = self.inv_weight_quant[0]
        self.active_inv_aquant  = self.inv_act_quant[0]
        self.active_sep_wquant  = self.sep_weight_quant[0]
        self.active_sep_aquant  = self.sep_act_quant[0]
    ...
    def forward(self, x):
        ...
        if self.inverted_bottleneck is not None:
            self.inverted_bottleneck.conv.active_out_channel = \
                make_divisible(round(in_channel * self.active_expand_ratio), MyNetwork.CHANNEL_DIVISIBLE)
            self.inverted_bottleneck.conv.active_weight_quant = self.active_inv_wquant
            self.inverted_bottleneck.conv.active_act_quant = self.active_inv_aquant
        # Pass down active_kernel_size
        self.depth_conv.conv.active_kernel_size = self.active_kernel_size
        self.depth_conv.conv.active_weight_quant = self.active_sep_wquant
        self.depth_conv.conv.active_act_quant = self.active_sep_aquant
        # Pass down active_out_channel
        self.point_linear.conv.active_out_channel = self.active_out_channel
        self.point_linear.conv.active_weight_quant = self.active_inv_wquant
        self.point_linear.conv.active_act_quant = self.active_inv_aquant
    ...
    def module_str(self):
        if self.use_se:
            return f"SE(O{self.active_out_channel}, E{round(self.active_expand_ratio, 1)}, K{self.active_kernel_size}, IW:{self.active_inv_wquant}, IA:{self.active_inv_aquant}, SW:{self.active_sep_wquant}, SA{self.active_sep_aquant})"
        else:
            return f"(O{self.active_out_channel}, E{round(self.active_expand_ratio, 1)}, K{self.active_kernel_size}, IW:{self.active_inv_wquant}, IA:{self.active_inv_aquant}, SW:{self.active_sep_wquant}, SA:{self.active_sep_aquant})"

    @property
    def config(self):
        return {
            'name': DynamicMBConvLayerQuant.__name__,
            'in_channel_list': self.in_channel_list,
            'out_channel_list': self.out_channel_list,
            'kernel_size_list': self.kernel_size_list,
            'expand_ratio_list': self.expand_ratio_list,
            ...
            'act_func': self.act_func,
            'use_se': self.use_se,
            'inv_weight_quant': self.inv_weight_quant,
            'inv_act_quant': self.inv_act_quant,
            'sep_weight_quant': self.sep_weight_quant,
            'sep_act_quant': self.sep_act_quant 
        }

    @staticmethod
    def build_from_config(config):
        return DynamicMBConvLayerQuant(**config)
    ...
    def get_active_subnet(self, in_channel, preserve_weight=True):
        ...
        if sub_layer.inverted_bottleneck is not None:
            ...
            copy_fake_quant(
                ...
                self.inverted_bottleneck.act_quant_list[sub_layer.inverted_bottleneck.act_quant_type]
            )
        # Seperable Convolution
        sub_layer.depth_conv.conv.weight.data.copy_(
            self.depth_conv.conv.get_active_filter(middle_channel, self.active_kernel_size).data
        ...
        copy_fake_quant(
            ...
            self.depth_conv.act_quant_list[sub_layer.depth_conv.act_quant_type]
        )
        # SE
        if self.use_se:
            se_mid = make_divisible(middle_channel // SEModule.REDUCTION, divisor=MyNetwork.CHANNEL_DIVISIBLE)
        ...
        copy_fake_quant(
            ...
            self.point_linear.act_quant_list[sub_layer.point_linear.act_quant_type]
        )
        return sub_layer
    ...
    def get_active_subnet_config(self, in_channel):
        return {
            ...
            'act_func': self.act_func,
            'use_se': self.use_se,
            'inv_weight_quant': self.active_inv_wquant,
            'inv_act_quant': self.active_inv_aquant,
            'sep_weight_quant': self.active_sep_wquant,
    ...
    def re_organize_middle_weights(self, expand_ratio_stage=0):
        ...
        if isinstance(self.depth_conv.conv.bn, DynamicGroupNorm):
            channel_per_group = self.depth_conv.conv.bn.channel_per_group
            importance_chunks = torch.split(importance, channel_per_group)
            for chunk in importance_chunks:
                chunk.data.fill_(torch.mean(chunk))
            importance = torch.cat(importance_chunks, dim=0)
        ...
        # DEPTH CONV
        adjust_bn_according_to_idx(self.depth_conv.conv.bn, sorted_idx)
        for fake_quant in self.depth_conv.weight_quant_list:
            if hasattr(fake_quant.activation_post_process, 'min_vals'): # Per Channel
                adjust_fake_quant_according_to_idx(fake_quant, sorted_idx)
        ...
        # INVERTED BOTTLENECK
        if self.inverted_bottleneck is not None:
            adjust_bn_according_to_idx(self.inverted_bottleneck.conv.bn, sorted_idx)
            for fake_quant in self.inverted_bottleneck.weight_quant_list:
                if hasattr(fake_quant.activation_post_process, 'min_vals'): # Per channel
                    adjust_fake_quant_according_to_idx(fake_quant, sorted_idx)
            self.inverted_bottleneck.conv.conv.weight.data = torch.index_select(
                self.inverted_bottleneck.conv.conv.weight.data, 0, sorted_idx
