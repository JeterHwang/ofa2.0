...
class SEModule(nn.Module):
	...
	def __init__(self, channel, reduction=None, channel_divisible=8):
		super(SEModule, self).__init__()

		self.channel = channel
		self.reduction = SEModule.REDUCTION if reduction is None else reduction

		num_mid = make_divisible(self.channel // self.reduction, divisor=channel_divisible)

		self.fc = nn.Sequential(OrderedDict([
			####################################
			## TODO : nn.AdaptiveAvgPool2d(1) ##
			####################################
			('reduce', nn.Conv2d(self.channel, num_mid, 1, 1, 0, bias=True)), 
			## Same as nn.linear(self.channel, num_mid, bias=False)
			('relu', nn.ReLU(inplace=True)),
			('expand', nn.Conv2d(num_mid, self.channel, 1, 1, 0, bias=True)),
			## Same as nn.linear(num_mid, self.channel, bias=False)
